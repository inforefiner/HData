package com.github.stuxuhai.hdata.plugin.reader.socket;

public class SocketReaderProperties {

    public static final String RECEIVER_BIND = "receiver.bind";

    public static final String RECEIVER_PORT = "receiver.port";

    public static final String OPERATE_TYPE = "operate.type";

    public static final String OPERATE_REGEX = "operate.regex";

    public static final String OPERATE_REGEX_MAPPING = "operate.regex.mapping";

    public static final String EXTRACT_COLUMNS = "extract.columns";

    public static final String NULL_STRING = "null.string";

    public static final String DEFAULTS_MAP = "defaults.map";
}
